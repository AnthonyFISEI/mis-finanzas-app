...
const balances = computed(() => {
  ...
  props.transacciones.forEach((t) => {
    ...
    const esDespuesDelCorte =
      esCredito && t.dia_corte && diaCompra > t.dia_corte;
...
<template>
  <div class="grid grid-cols-1 md:grid-cols-2 gap-4 my-6">
    <div
      class="bg-gradient-to-br from-white to-gray-50 p-5 rounded-xl border border-gray-200 shadow-sm"
    >
      ...
      <p class="text-3xl font-black text-gray-900">
        ${{ balances.inmediato.toFixed(2) }}
      </p>
      <p class="text-[10px] text-gray-400 mt-2 italic">
        Gastos que debes cubrir en el mes actual.
      </p>
    </div>

    <div
      class="bg-gradient-to-br from-white to-blue-50 p-5 rounded-xl border border-blue-100 shadow-sm"
    >
      <div class="flex items-center gap-2 mb-2">
        <span class="w-2 h-2 rounded-full bg-blue-500 animate-pulse"></span>
        <h3 class="text-xs font-bold text-blue-600 uppercase tracking-widest">
          Gastos para el Próximo Mes
        </h3>
      </div>
      <p class="text-3xl font-black text-blue-700">
        ${{ balances.proyectado.toFixed(2) }}
      </p>
      <p class="text-[10px] text-blue-400 mt-2 italic">
        Consumos a cubrir para el siguiente mes.
      </p>
    </div>
  </div>
</template>
